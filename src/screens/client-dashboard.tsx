...
// --- Componente SaleCard (CON CORRECCIONES DE SCOPE Y LÓGICA) ---
...
    isOffline: boolean; // ✅ AHORA EN SCOPE
}) => {
...
    
    // ✅ CORRECCIÓN: Chequeo defensivo para bloquear la navegación offline.
    const handleEdit = useCallback((e: any) => {
        e.stopPropagation();
        // Bloqueo de edición si está offline.
        if (isOffline) {
            Toast.show({
                type: 'info',
...
            activeOpacity={0.8}
            // ✅ CORREGIDO: Mantenemos el principal Touchable Opacity activo
        >
...
                            disabled={isOffline} // Mantenemos para accesibilidad/visual
                        >
                            {/* ✅ CORRECCIÓN: Color del icono condicional */}
                            <Feather name="edit" size={22} color={isOffline ? COLORS.textSecondary : COLORS.primary} />
                        </TouchableOpacity>
                        <TouchableOpacity
...
    
    // --- ✅ CORRECCIÓN: Obtenemos el setter y la nueva función del contexto ---
    const { 
        clients, 
        sales, 
        rubros, 
...
        isOffline,
        setSalesState,
        deleteSaleAndRevertStock // ✅ CLAVE: Nueva función para eliminar y revertir stock
    } = useData();
    const [isDeleting, setIsDeleting] = useState(false);
    
    // ✅ CORRECCIÓN: Usamos el setter del contexto con el nombre que la función ya espera
...
    // --- handleDeleteSale (CORREGIDO: Usa la función del contexto para revertir stock) ---
    const handleDeleteSale = useCallback(async (saleId: string) => {
        if (isDeleting || !saleId) return;

        // 🛑 CLAVE: Buscar la venta para obtener los items ANTES de la mutación local
...
                        setIsDeleting(true);
                        
                        // 1. MUTACIÓN OPTIMISTA: Eliminar de la UI (El stock local se revierte en la función de Contexto)
                        setSalesState((prevSales: Sale[]) => prevSales.filter(s => s.id !== saleId));
                        
                        try {
                            // 2. Llamar a la función del Contexto para reintegrar stock LOCAL y eliminar REMOTO.
                            // Esto asume que la Cloud Function corregirá el stock en Firebase.
...
                            Toast.show({ type: 'error', text1: 'Error', text2: 'No se pudo eliminar la venta.', position: 'bottom' });
                            ...
                            // 3. Rollback: Reinsertar la venta si falla la eliminación remota
                            setSalesState((prevSales: Sale[]) => [...prevSales, saleToDelete]); 
                            
                            // El rollback del stock local (si falla la eliminación) ya está dentro de deleteSaleAndRevertStock.
                        } finally {
                            setIsDeleting(false);
                        }
                    }
...
        marginLeft: 8,
    },
    actionButtonDisabled: { // ✅ Estilo para deshabilitar visualmente
        opacity: 0.4,
    },
});
